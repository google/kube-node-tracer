...
#
# Description: Wrapper around `tcpdump` to perform rolling packet captures on the primary interface.
...
output_dir='./dump/'        #The mountPath directory for Kubernetes DaemonSet. Note this is not file name as that is based on the node name.
...
nodename=''    # Name of Kubernetes worker node -- this is retreived from the cluster metadata
