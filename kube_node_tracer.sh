...
#Script arguments
mountPath='./dump/'                             #Intermediary output path for pcap files which should be mounted to a GKE persistent storage volume as to not consume cluster resources
gcsbucket=''                                    #Google Cloud Storage bucket 
nodename=''                                     #Kubernetes node name (ex. gke-node-test)
compress='true'                                 #Optional cpression using Gzip

##tcpdump arguments  
interface='X'                                   #Network interface -- GKE only uses a single interface for its nodes
...
if [ -z "$nodename" ]; then 
    echo "A Kubernetes node name must be supplied. Killing process."
    exit 1
fi

if [ -z "$gcsbucket" ]; then 
    echo "A Google Cloud Storage bucket must be avaliable to store pcap files. Killing process."
    exit 1
fi
...
if [ -z "$rotate_seconds" ]; then 
    bash ./rolling_tcpdump.sh -o $mountPath -N $nodename -i $interface $filter -s $snaplen -B $buffer_size -C $filesize_MB -W $filecount
else
    bash ./rolling_tcpdump.sh -o $mountPath -N $nodename -i $interface $filter -s $snaplen -B $buffer_size -C $filesize_MB -W $filecount -G $rotate_seconds
fi